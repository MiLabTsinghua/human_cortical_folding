...
run_TF_module_GSEA <- function(TF_score_list, gene_modules,minsize = 20) {
    module_results <- lapply(names(TF_score_list), function(tf) {
      scores <- TF_score_list[[tf]]
      scores <- sort(scores, decreasing = TRUE)

      fgseaRes <- fgsea(
          pathways = gene_modules,
          stats = scores,
            scoreType = 'pos',
          minSize = minsize,
        )
...
PrepareNet  <- function(regulon) {
	raw_links <- lapply(names(regulon), function(ctp) {
		...
		ctp_link <- lapply(ctp_regu, function(xx) {
			regu <- xx[-1]
			edges <- data.frame(from = xx[1], to = names(regu), weight = regu, stringsAsFactors = FALSE) %>%
					mutate(subtype = ctp)
			return(edges)
			}) %>%
			do.call(rbind, .)
    ...
    raw_links$weight <- as.numeric(raw_links$weight)
...
plot_tfnet_signal_final <- function(netpre_object, 
    ...
    if (!is.null(vertice_anno_shape)){
        for (TF in names(vt.shape)){
            if ((netpre_object$meta[TF,'gtype']=='target')){
                vt.shape[[TF]] <- ifelse(TF %in% vertice_anno_shape[[1]],'square','circle')
            }
        }
    }
    # vt.shape[['NKX3-1']] = "pie"
	...
	circle.cols <- ifelse(vt.shape %in% c("circle","square"), 
	...
	E(net)$arrow.size <- ifelse(1:length(E(net)) %in% ed_hl_idx, 0.7, 0.2)
	...
	if (redo_layout | !file.exists(paste0(output_dir,"/load_files/", file_name, "_layout.rds"))){
            e <- get.edgelist(net, names=FALSE)
			l <- qgraph.layout.fruchtermanreingold(e,vcount=vcount(net), 
							area=1*(vcount(net)^layout_area), repulse.rad=(vcount(net))*layout_pulse) 
			lmeta <- as.data.frame(l)
			lmeta$keep <- ifelse(1:nrow(lmeta) %in% rm_idx, 0, 1)
			saveRDS(lmeta, file = paste0(output_dir,"/load_files/", file_name, "_layout.rds"))
            ...
            lmeta <- readRDS(paste0(output_dir,"/load_files/", file_name, "_layout.rds"))
            l <- lmeta[,c('V1','V2')] %>% as.matrix()
			# l <- qgraph.layout.fruchtermanreingold(e,vcount=vcount(net2), 
			# 				area=1*(vcount(net2)^layout_area), repulse.rad=(vcount(net2))*layout_pulse)
    ...
    if (!is.null(pie_custom)){
        network_TFs <- names(pie_custom)[names(pie_custom) %in% names(vt_anno_list)]
        for (TF in network_TFs){
            vt_anno_list[[TF]] <- pie_custom[[TF]]
        }
    }
	...
	plot(net2, edge.curved=0, 
			...
			vertex.label.dist = 0, 
			rescale = TRUE)
	legend("bottomleft", inset=.02, title="Colors", c(vt_anno_names, "EMPTY"), fill=vertice_cate_cols, cex=1.5, ncol = 4)
	dev.off()
} 
